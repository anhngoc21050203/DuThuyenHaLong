<?php
  ...
  if(isset($_GET['id'])){
    $id_cate = $_GET['id'];
    $edit = $category->get_duthuyen_cate($id_cate);
  }
  if($id_cate){
    $rs = $edit->fetch_assoc();
  }
  if(isset($_POST['update'])){
    ...
    if (!empty($_FILES['thumbnail']['tmp_name'])) {
      ...
      if($username != null ){
      $add_list = $category->update_du_thuyen($username, $thumbnail, $content, $id);
    }
  }
...
<section class="section">
  <div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
              <form class="row g-3" style="margin-top: 10px;" action="cate_edit.php" method="post" enctype="multipart/form-data">
                <div class="col-12">
                  <label for="inputNanme4" class="form-label">Tên danh mục</label>
                  <input type="text" class="form-control" id="inputNanme4" name="namecate" value="<?php echo $rs['namecate']  ?>">
                </div>
                <div class="col-md-12">
